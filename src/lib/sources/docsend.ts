...
import { InvalidURLError, EmailGateError, ExtractionError } from "../types.js";
import { launchBrowserContext, tryPassEmailGate } from "./base.js";
...
export const docsendSource: DeckSource = {
  id: "docsend",
  name: "DocSend",
  exampleUrl: "https://docsend.com/view/XXXXXX",

  canHandle(url: string): boolean {
    return DOCSEND_URL_PATTERN.test(url);
  },
  ...
  async extractSlideUrls(url: string, options: ExtractOptions): Promise<DeckInfo> {
    const { headless, profileKey, gateEmail, debug = false, onStatus } = options;

    const report = (msg: string) => onStatus?.(msg);

    let slug: string | null = this.parseIdentifier(url);

    const usePersistent = profileKey !== null && hasBrowserProfile(profileKey);
    const profileDir = usePersistent ? getBrowserProfileDir(profileKey) : undefined;
    ...
    for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
      ...
      const batchEnd = Math.min(batchStart + PAGE_DATA_BATCH_SIZE, slideCount);
